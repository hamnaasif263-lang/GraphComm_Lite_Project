"""
Prostate Cancer Correlation Analysis
Analysis of IGF1R-MYC-CCND-p27 relationships in real prostate cancer data
"""
...
class ProstateCancerCorrelationAnalysis:
    """Perform correlation analysis on prostate cancer single-cell data"""
    ...
    def load_prostate_cancer_data(self):
    ...
    def create_correlation_heatmap(self, sc_data):
        ...
        ax.set_title('Prostate Cancer: Correlation Matrix (n=280,000 cells)', 
                     fontsize=14, fontweight='bold', pad=20)
        
        plt.tight_layout()
        fig.savefig(self.output_base / 'pc_correlation_heatmap.png',
                   dpi=self.dpi, bbox_inches='tight', facecolor='white', format='png')
        print(f"  [SAVED] pc_correlation_heatmap.png ({fig.get_size_inches()[0]:.1f}x{fig.get_size_inches()[1]:.1f}\")")
        plt.close()
    ...
    def create_scatter_plots(self, sc_data):
        ...
        fig.suptitle('Prostate Cancer: Phenotype Correlation Analysis (n=280,000 cells)',
        ...
        plt.tight_layout()
        try:
            fig.savefig(self.output_base / 'pc_scatter_correlations.png',
                       dpi=150, bbox_inches='tight', facecolor='white', format='png')
            print(f"  [SAVED] pc_scatter_correlations.png")
        except Exception as e:
    ...
    def save_correlation_matrix(self, sc_data):
        ...
        output_path = self.output_base / 'pc_correlation_matrix.csv'
        corr_matrix.to_csv(output_path)
        print(f"  [SAVED] pc_correlation_matrix.csv")
    ...
    def run_analysis(self):
        ...
        print("\n" + "="*80)
        print("PROSTATE CANCER CORRELATION ANALYSIS - REAL DATA")
        print("="*80)
        
        # Load data
        sc_data = self.load_prostate_cancer_data()
...
if __name__ == "__main__":
    analyzer = ProstateCancerCorrelationAnalysis()
