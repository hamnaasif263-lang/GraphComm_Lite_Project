"""
Professional Visualization Suite for Comparative Cancer Analysis - Fixed Version
...
class ProfessionalComparativeVisualizer:
    ...
    def create_dormancy_comparison(self, dormancy_data):
        ...
        for cancer in sorted(dormancy_data.keys()):
            ...
            # Extract percentages from aggregated data
            if 'dormant_pct' in data.columns:
                dormancy_pct = data['dormant_pct'].values[0]
            else:
                dormancy_pct = 0
                
            if 'transitional_pct' in data.columns:
                transitional_pct = data['transitional_pct'].values[0]
            else:
                transitional_pct = 0
                
            if 'proliferative_pct' in data.columns:
                proliferative_pct = data['proliferative_pct'].values[0]
            else:
                proliferative_pct = 0
    ...
    def create_igf_dormancy_correlation(self, igf_data, dormancy_data):
        ...
        # Add trend line
        if len(igf_activations) > 1 and len(set(igf_activations)) > 1:
            try:
    ...
    def create_summary_table_figure(self, igf_data, dormancy_data):
        """Generate summary statistics as professional table figure"""
        fig, ax = plt.subplots(figsize=(14, 5))
        ax.axis('tight')
        ax.axis('off')
        
        cancer_names = ['Breast Cancer', 'Lung Cancer', 'Prostate Cancer']
        table_data = []
        ...
        for cancer in sorted(igf_data.keys()):
            ...
            table_data.append([
                cancer.replace('_', ' ').title(),
                f'{igf_activation:.3f}',
                f'{dormancy_pct:.1f}%',
                f'{igf_df["mTOR_Mean"].values[0]:.3f}',
                f'{igf_df["MAPK_Mean"].values[0]:.3f}',
                f'{igf_df["MYC_Mean"].values[0]:.3f}'
            ])
        
        table = ax.table(cellText=table_data,
                        colLabels=['Cancer Type', 'IGF Activation', 'Dormant%', 'mTOR', 'MAPK', 'MYC'],
                        cellLoc='center',
                        loc='center',
        ...
        plt.title('Comparative Analysis Summary Statistics',
                 fontsize=14, fontweight='bold', pad=20)
        ...
        fig.savefig(self.comparative_dir / 'Summary_Statistics_Table.png',
                   dpi=self.dpi, bbox_inches='tight', facecolor='white')
        plt.close()
        print("[OK] Summary Statistics Table")
    ...
    def generate_all_visualizations(self):
        ...
        try:
            ...
            self.create_summary_table_figure(igf_data, dormancy_data)
